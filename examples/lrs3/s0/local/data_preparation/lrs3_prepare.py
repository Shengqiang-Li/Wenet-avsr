...
def get_file_label(lrs3_root):
    ...
    for split in ['trainval', 'test', 'pretrain']:
        subdirs = os.listdir(os.path.join(lrs3_root, split))
        for subdir in tqdm(subdirs):
            video_fns = glob.glob(os.path.join(lrs3_root, split, subdir, '*jpg'))
    ...
    #     labels_total.append(label)
    video_id_fn, label_fn = os.path.join(lrs3_root, 'file_jpg.list'), os.path.join(lrs3_root, 'label_jpg.list')
